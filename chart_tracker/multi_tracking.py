...
def getMelon(song_title, melon_sid):
    ...
    for item in tr_list_100 :
        if target_song in item.select(".wrap_song_info")[0].text :
    ...
    for item in tr_list_30 :
        if song_title in item.select(".wrap_song_info")[0].text :
            곡명 = item.select('.wrap_song_info a')[0].text
            아티스트 = item.select('.wrap_song_info a')[1].text
            순위 = item.select('.rank')[0].text
            변동 = item.select('.rank_wrap')[0].attrs['title']
...
def getGenie(song_title, genie_sid):
    ...
    for item in rank_list :
        if song_title in item.select(".title")[0].text :
...
def getBugs(song_title, bugs_sid):
